...
<div class="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
    ...
    <form action="{{ route('recettes.update', $recipe->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" class="w-full h-28 px-4 py-2 border rounded-lg">{{ old('description', $recipe->description) }}</textarea>
        </div>

        <!-- Affichage de l'image actuelle -->
        @if($recipe->imageUrl)
        <div class="mb-4">
            <label class="block text-lg font-semibold text-gray-600">Image actuelle</label>
            <img src="{{ asset('storage/' . $recipe->imageUrl) }}" alt="Image de la recette {{ $recipe->titre }}" class="max-w-xs rounded-lg shadow-lg">
        </div>
        @endif

        <!-- Champs pour télécharger une nouvelle image -->
        <div class="mb-4">
            <label for="imageUrl" class="block text-lg font-semibold text-gray-600">Modifier l'image de la recette</label>
            <input type="file" name="imageUrl" id="imageUrl" class="w-full px-4 py-2 border rounded-lg">
            <span class="text-sm text-gray-500">Laissez vide pour conserver l'image actuelle.</span>
        </div>
            ...
            <div id="selected-ingredients" class="mb-6">
                ...
                <div class="flex items-center mb-2 selected-ingredient" data-ingredient-id="{{ $ingredient->id }}">
                    ...
                    <select name="unites[{{ $ingredient->id }}]" class="ml-4 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="g" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'g' ? 'selected' : '' }}>g</option>
                        <option value="kg" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'kg' ? 'selected' : '' }}>kg</option>
                        <option value="ml" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'ml' ? 'selected' : '' }}>ml</option>
                        <option value="cl" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'cl' ? 'selected' : '' }}>cl</option>
                        <option value="l" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'l' ? 'selected' : '' }}>l</option>
                        <option value="cuillère à soupe" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'cuillère à soupe' ? 'selected' : '' }}>cuillère à soupe</option>
                        <option value="cuillère à café" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'cuillère à café' ? 'selected' : '' }}>cuillère à café</option>
                        <option value="unité" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'unité' ? 'selected' : '' }}>unité</option>
                        <option value="feuille" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'feuille' ? 'selected' : '' }}>feuille</option>
                        <option value="tranche" {{ old("unites.{$ingredient->id}", $ingredient->pivot->quantite_unite) == 'tranche' ? 'selected' : '' }}>tranche(s)</option>
                    </select>
